...
instance_meta=$(curl -H Metadata:true "http://169.254.169.254/metadata/instance?api-version=2019-03-11" | jq .)
subscriptionId=$(echo ${instance_meta} | jq -r .compute.subscriptionId)
resourceGroupName=$(echo ${instance_meta} | jq -r .compute.resourceGroupName)
